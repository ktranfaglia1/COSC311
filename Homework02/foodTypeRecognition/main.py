...
This program reads in a food type dataset, randomly splits the dataset into two parts: 80% for training and 20% for
testing, and uses various classification algorithms in attempt to obtain the highest testing accuracy on the testing
data. A classification Report is used to show the classification performance and a heatmap is used to show the
...
foodData.columns = ["column" + str(i + 1) for i in range(0, len(foodData.columns) - 1)] + ["Target"]
...
attributeTrain, attributeTest, targetTrain, targetTest = train_test_split(attributes, target, test_size=.2,
